...
var RoomEngine = class {
  ...
  restorePersistentState(value) {
    ...
    const state = value;
    if (state.version !== 1 || typeof state.playerDirections !== "object" || state.playerDirections === null) return;
  ...
  setMoveTarget(userId, connectionId, clientSequence, rawTarget) {
    const player = this.players.get(userId);
  ...
  cancelMove(userId, connectionId, clientSequence) {
    const player = this.players.get(userId);
    if (!player || player.connectionId !== connectionId) return "stale_connection";
    if (!Number.isSafeInteger(clientSequence) || clientSequence <= player.lastClientSequence) {
      return "stale_sequence";
...
var PlanetRoom = class {
  ...
  async fetch(request) {
    ...
    this.engine.connect(userId, connectionId);
  ...
  broadcastSnapshot() {
    this.broadcast(this.engine.createSnapshot(Date.now()));
  }
...
var src_default = {
  async fetch(request, env) {
    ...
    if (request.method === "OPTIONS" && url.pathname === "/v1/dev/sessions") {
    ...
    const match = url.pathname.match(/^\/v1\/planets\/([^/]+)\/connect$/);
...
// .wrangler/tmp/bundle-YbyXBB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YbyXBB/middleware-loader.entry.ts
